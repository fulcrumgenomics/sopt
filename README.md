...
The `Tool` trait is not required, but it is generally useful to have a trait which is implemented by all commands.  Alternatively you could invoke `Sopt.parseCommand[AnyRef]`!  To do so, you would need to manually construct the list of commands instead of searching for subclasses with `Sopt.find[AnyRef](packages)`.

The `Main` class is implemented only once and acts as a dispatcher to any number of commands that implement `Tool`.  It also acts as a central point to add consistent behaviour amongst tools, for example printing out the date and time at the start and end of execution.
